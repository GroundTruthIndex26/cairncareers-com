...
- API discovery: the Worker serves `/.well-known/api-catalog` (RFC 9727, `API_CATALOG` in `worker/index.ts`) and `GET /api/health`. It points to `client/public/openapi.json` and `client/public/docs/api.md`. Change an endpoint in the Worker and update both files in the same commit. It also runs a read-only MCP server at `/mcp` (tools `list_pages`, `get_page`, `get_beta_count`; `routeMcp` in `worker/index.ts`), with its server card at `/mcp/server-card` and `/.well-known/mcp/server-card.json` and an entry in `/.well-known/ai-catalog.json`. `list_pages` reads `client/public/llms.txt`, so a page missing from llms.txt is missing for agents too. Keep it read-only unless Brooke decides otherwise.
