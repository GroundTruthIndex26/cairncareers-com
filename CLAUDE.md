...
- API discovery: the Worker serves `/.well-known/api-catalog` (RFC 9727, `API_CATALOG` in `worker/index.ts`) and `GET /api/health`. It points to `client/public/openapi.json` and `client/public/docs/api.md`. Change an endpoint in the Worker and update both files in the same commit.
